...
if(getgenv().vizitems==nil)then getgenv().vizitems="{'HumanoidRootPart','Torso','Left Arm','Right Arm','Left Leg','Right Leg','Head'}";end;
        ...
        xpcall(function()
            ...
            socket.OnMessage:Connect(function(msg)
                ...
                elseif(spi[1]=="data")and(spi[2]=="lagviz")and(spi[3]==game:GetService("Players").LocalPlayer.Name)and(spi[4]~=game:GetService("Players").LocalPlayer.Name)then 
                    print(spi[6]);
                    local ritems=toTable(spi[5],getgenv().vizitems);
...
if((getgenv().hostuser~=nil)and(getgenv().hostuser~=game:GetService("Players").LocalPlayer.Name))or(getgenv().hostuser==nil)or(getgenv().hostuser=="nil")then 
    spawn(function()
        while(getgenv().testcdesync==true)do 
            xpcall(function()
                if(socket~=nil)then 
                    local ttbl=toTable(getgenv().vizitems,{'HumanoidRootPart','Torso','Left Arm','Right Arm','Left Leg','Right Leg','Head'});
                    for b,a in pairs(currentUsers)do 
                        if(a~=nil)and(a.Parent~=nil)and(a.Character~=nil)and(a.Name~=game:GetService("Players").LocalPlayer.Name)then 
                            local data="{";
                            for _,item in pairs(ttbl)do 
                                local part=a.Character:FindFirstChild(item);
                                if(part~=nil)and(part:IsA("BasePart"))then 
                                    ...
                                    data=data.."['"..item.."'] = {Position = Vector3.new("..tostring(pos).."), Rotation = Vector3.new("..tostring(rot).."), Size = Vector3.new("..tostring(sz)..")}, "
                                end;
                            end;
                            ...
                            local torsorot=CFrame.new(0,0,0);
                            local torsosz=Vector3.new(2,2,1);pcall(function()torsopos=a.Character.Torso.Position;torsorot=a.Character.Torso.Rotation;torsosz=a.Character.Torso.Size;end);
                            local gdata="{['HumanoidRootPart'] = {Position = Vector3.new("..string.format("%3i, %3i, %3i",hrppos.X,hrppos.Y,hrppos.Z).."), Rotation = Vector3.new("..string.format("%3i, %3i, %3i",hrprot.X,hrprot.Y,hrprot.Z).."), Size = Vector3.new("..tostring(string.format("%3i, %3i, %3i",hrpsz.X,hrpsz.Y,hrpsz.Z))..")}, ['Torso'] = {Position = Vector3.new("..string.format("%3i, %3i, %3i",torsopos.X,torsopos.Y,torsopos.Z).."), Rotation = Vector3.new("..string.format("%3i, %3i, %3i",torsorot.X,torsorot.Y,torsorot.Z).."), Size = Vector3.new("..tostring(string.format("%3i, %3i, %3i",torsosz.X,torsosz.Y,torsosz.Z))..")}}"
                            if(lastdatas[a.Name]~=gdata)then 
                                lastdatas[a.Name]=gdata;
                                socket:Send("data´lagviz´"..a.Name.."´"..game:GetService("Players").LocalPlayer.Name.."´"..getgenv().vizitems.."´"..data);
                            end;
