...
game:GetService("Players").LocalPlayer.OnTeleport:Connect(function(status)
	if(status==Enum.TeleportState.InProgress)and(queue_teleport~=nil)then 
		queue_teleport("getgenv().total_slaps="..total_gained..";getgenv().last_servers={\""..table.concat(getgenv().last_servers,"\",\"").."\"};wait(0.5);if(game:IsLoaded()==false)then game.Loaded:Wait();end;loadstring(game:HttpGet(\"https://raw.githubusercontent.com/Arales03482/My-Roblox-Scripts/main/SB%20Auto%20Farm%20Slapples.lua\"))();");
	end;
end);
wait(0.75);
